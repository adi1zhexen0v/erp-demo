...
export default function CompletionActCreateModal({
    ...
    if (watchedServiceItem !== previousServiceItemRef.current) {
      ...
      if (watchedServiceItem) {
        ...
        if (service) {
          ...
          const amountToSet = Number.isInteger(remaining) ? String(Math.floor(remaining)) : formatPrice(remaining.toString());
  ...
  async function onFormSubmit(data: typeof control._defaultValues) {
    ...
    if (selectedService && calculatedRemaining !== null && requestedAmount > calculatedRemaining) {
      setToast({
        color: "negative",
        text: `${t("completionAct.summary.exceededRemainder")}. ${t("completionAct.fields.remainingAmount")}: ${formatPrice(calculatedRemaining)} ₸`,
      });
      return;
    }
    ...
    try {
      ...
      setToast({
        color: "negative",
        text: t("completionAct.summary.createError"),
      });
    }
  }
          ...
          <form onSubmit={handleSubmit(onFormSubmit)} className="flex flex-col gap-4">
              ...
              <div className="p-5 radius-lg surface-component-fill">
                <div className="flex flex-col gap-3">
                  <div className="flex justify-between items-center">
                    <span className="text-label-sm content-base-secondary">{t("completionAct.summary.totalServiceAmount")}</span>
                  ...
                  {usedAmount !== null && usedAmount > 0 && (
                    <div className="flex justify-between items-center">
                      <span className="text-label-sm content-base-secondary">{t("completionAct.summary.alreadyCalculated")}</span>
                  ...
                  <div className="flex justify-between items-center pt-3 border-t surface-base-stroke">
                    <span className="text-label-sm content-base-secondary">{t("completionAct.summary.remainingToCalculate")}</span>
                    <Badge
                      variant="soft"
                  ...
                  {requestedAmount > 0 && calculatedRemaining < requestedAmount && (
                    <p className="text-label-md content-action-negative mt-1">
                      {t("completionAct.summary.exceededRemainder")} {formatPrice(requestedAmount - calculatedRemaining)} ₸
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-auto pt-4">
              ...
              <Button
                ...
                variant="primary"
                size="lg"
                disabled={isLoading || !isServiceSelected || isFullyUsed}>
