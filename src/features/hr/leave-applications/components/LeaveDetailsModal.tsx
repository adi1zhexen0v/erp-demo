...
export default function LeaveDetailsModal({ leave, onClose }: Props) {
  ...
  async function handleUpdateCertificate() {
    if (!certificateFile || isUpdatingCertificate) return;
  ...
  async function handleDownloadApplicationDocx() {
    ...
    try {
      ...
      if (leaveType === "annual") {
        ...
        fileName = t("downloads.annualLeaveApplicationFileName", { leaveId });
      } else if (leaveType === "unpaid") {
        blob = await previewUnpaidApp(leaveId).unwrap();
        fileName = t("downloads.unpaidLeaveApplicationFileName", { leaveId });
      } else {
        blob = await previewMedicalApp(leaveId).unwrap();
        fileName = t("downloads.medicalLeaveApplicationFileName", { leaveId });
      }
      downloadBlob(blob, fileName);
    } catch {
      setPrompt({
        title: t("messages.errorTitle"),
        text: t("actions.downloadApplicationDocxError"),
        variant: "error",
      });
    }
  }
  ...
  async function handleDownloadOrderDocx() {
    ...
    try {
      ...
      if (leaveType === "annual") {
        ...
        fileName = t("downloads.annualLeaveOrderFileName", { leaveId });
      } else if (leaveType === "unpaid") {
        blob = await previewUnpaidOrder(leaveId).unwrap();
        fileName = t("downloads.unpaidLeaveOrderFileName", { leaveId });
      } else {
        blob = await previewMedicalOrder(leaveId).unwrap();
        fileName = t("downloads.medicalLeaveOrderFileName", { leaveId });
      }
      downloadBlob(blob, fileName);
    } catch {
      setPrompt({
        title: t("messages.errorTitle"),
        text: t("actions.downloadOrderDocxError"),
        variant: "error",
      });
    }
  }
