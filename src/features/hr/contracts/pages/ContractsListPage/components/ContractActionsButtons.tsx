...
import { Button, Dropdown, DropdownItem, PromptForm } from "@/shared/ui";
    ...
    <div className="flex flex-col gap-2 pt-3 border-t surface-base-stroke">
      {actions.map((action) => {
        ...
        return (
          <Button
            ...
            onClick={async (e) => {
              e.stopPropagation();
              if (action.id === "submit_for_signing") {
      ...
      {showConfirmSubmit && (
        <PromptForm
          ...
          onConfirm={async () => {
            await onAction("submit_for_signing", contract.id, contract);
            setShowConfirmSubmit(false);
          }}
          title={t("prompt_form.title")}
          text={t("prompt_form.description")}
          isLoading={isThisContractSubmitting}
