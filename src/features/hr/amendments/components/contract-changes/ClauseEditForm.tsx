...
function replacePlaceholders(content: string, contractData: ContractDetailResponse, lang: "ru" | "kk"): string {
  let result = content;
  ...
  function replacePlaceholder(_match: string, placeholder: string): string {
    switch (placeholder) {
      ...
      case "work_city":
        ...
        return lang === "ru" ? contractData.work_city.name_ru : contractData.work_city.name_kk;

      case "work_city_ru":
        return contractData.work_city ? contractData.work_city.name_ru : "_____________";

      case "work_city_kk":
        return contractData.work_city ? contractData.work_city.name_kk : "_____________";
...
export default function ClauseEditForm({ employee, clause, contractData, onBack, onClose }: Props) {
  ...
  const initialContentRu = useMemo(() => {
    return replacePlaceholders(clause.content_ru, contractData, "ru");
  }, [clause.content_ru, contractData]);

  const initialContentKk = useMemo(() => {
    return replacePlaceholders(clause.content_kk, contractData, "kk");
