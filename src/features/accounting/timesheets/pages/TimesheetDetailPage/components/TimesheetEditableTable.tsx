...
import { getInitials, isValidCellValue, getCellStyle, calculateTotals, getDayOfWeek, type EntryStats } from "../../../utils";
...
export default function TimesheetEditableTable({
  ...
  const totals = useMemo(() => {
    const statsArray = sortedEntries.map((entry) => getEntryStats(entry.id));
    return calculateTotals(statsArray);
          ...
          <Table.Header variant="plain">
            <Table.Row className="border-none">
              ...
              {daysInMonth.map((day) => {
                ...
                const dayOfWeek = getDayOfWeek(dayNum, year, month, t);
                return (
                  <Table.HeadCell key={day} width="w-10" align="center" className="p-2!">
                    <div className="flex flex-col gap-1">
                      ...
                      <span className="text-label-xs content-action-neutral">{dayOfWeek}</span>
                    </div>
                  </Table.HeadCell>
                );
              })}
