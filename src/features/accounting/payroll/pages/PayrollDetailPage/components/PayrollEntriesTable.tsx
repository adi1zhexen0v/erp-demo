...
import { type Locale, formatMoneyKzt } from "@/shared/utils";
import type { PayrollEntry } from "@/features/accounting/payroll";
import { getInitials, getStatusBadges, getPayrollEntryTotals } from "@/features/accounting/payroll";
  ...
  if (entries.length === 0) {
    return (
      <div className="p-5 rounded-lg border surface-base-stroke text-center">
        <p className="text-body-regular-sm content-action-neutral">{t("detail.entries.empty")}</p>
      </div>
    );
  }
    ...
    <Table.Table>
      ...
      <Table.Body>
        {entries.map((entry, index) => {
          const totals = getPayrollEntryTotals(entry);
          ...
          return (
            <Table.Row key={entry.id}>
              ...
              <Table.Cell isBold>{formatMoneyKzt(entry.net_salary, locale)}</Table.Cell>
              <Table.Cell>{formatMoneyKzt(totals.totalTaxes, locale)}</Table.Cell>
