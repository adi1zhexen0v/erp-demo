...
export default function AccountingTab({ entry, locale, isFullScreen = false }: Props) {
  const { t, i18n } = useTranslation("PayrollPage");
  const currentLocale = (i18n.language as "ru" | "kk" | "en") || (locale as "ru" | "kk" | "en") || "ru";
      ...
      <div className={`grid gap-3 ${isFullScreen ? "grid-cols-3" : "grid-cols-1"}`}>
        <div className="p-5 radius-lg border surface-base-stroke flex flex-col gap-5 justify-between">
          <div className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-3">
              <div className="flex justify-between items-center">
                ...
                <p className="text-body-bold-md content-action-positive">+ {formatMoneyKzt(grossSalary, currentLocale)}</p>
              </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <p className="text-body-bold-md content-action-negative">
                  - {formatMoneyKzt(opv + ipn + vosms, currentLocale)}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="radius-sm p-3 surface-component-fill flex justify-between items-center">
            ...
            <span className="text-display-2xs content-base-primary">{formatMoneyKzt(totalEmployer, currentLocale)}</span>
          </div>
        </div>
        ...
        <div className="p-5 radius-lg border surface-base-stroke flex flex-col gap-5 justify-between bg-primary-500">
          <div className="flex flex-col gap-5">
            ...
            <div className="flex flex-col">
              <div className="flex items-center justify-between py-2 border-b border-white/20">
                ...
                <span className="text-body-bold-md text-white">{formatMoneyKzt(grossSalary, currentLocale)}</span>
              </div>
              <div className="flex items-center justify-between py-2 border-b border-white/20">
                <span className="text-label-xs text-white">{t("tabs.accounting.paid")}</span>
                <span className="text-body-bold-md text-white">{formatMoneyKzt(netSalary, currentLocale)}</span>
              </div>
              <div className="flex items-center justify-between py-2">
                ...
                <span className="text-body-bold-md text-white">
                  {formatMoneyKzt(opv + ipn + vosms + totalEmployer, currentLocale)}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="radius-sm p-3 bg-white flex justify-between items-center">
            ...
            <span className="text-display-2xs text-black">{formatMoneyKzt(grossSalary + totalEmployer, currentLocale)}</span>
          </div>
        </div>
      </div>
    </div>
