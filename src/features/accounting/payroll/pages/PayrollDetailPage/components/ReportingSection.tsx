...
import { ReceiptDiscount, Global, DocumentText, People, type Icon } from "iconsax-react";
...
function groupGPHPayments(payments: GPHPayment[], t: (key: string) => string): GroupedData[] {
  ...
  const total = payments.reduce(
    (acc, payment) => {
      acc.count++;
      acc.gross += toNumber(payment.gross_amount);
      acc.net += toNumber(payment.net_amount);
      return acc;
    },
...
function ReportCard({ title, data, icon: IconComponent }: ReportCardProps) {
        ...
        {data.map((row) => (
          <div
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-body-bold-md content-base-primary">{formatMoneyKzt(row.gross, locale)}</span>
            </div>
            <div className="flex items-center justify-between surface-base-fill p-3 radius-sm mt-2">
              <span className="text-body-bold-md content-action-neutral">{t("reportingSection.toPay")}</span>
              <span className="text-heading-lg content-action-positive">{formatMoneyKzt(row.net, locale)}</span>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="p-5 radius-lg border surface-component-stroke flex flex-col gap-5">
      ...
      <div className={gridClasses}>
        <ReportCard title={t("reportingSection.byTaxCategory")} data={byTaxCategory} icon={ReceiptDiscount} />
        <ReportCard title={t("reportingSection.byResidency")} data={byResidency} icon={Global} />
        <ReportCard title={t("reportingSection.byContractType")} data={byContractType} icon={DocumentText} />
        {hasGPH && <ReportCard title={t("reportingSection.byGPHPayments")} data={byGPHPayments} icon={People} />}
      </div>
    </div>
  );
}
