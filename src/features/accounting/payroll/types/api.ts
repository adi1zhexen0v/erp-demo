...
export interface PayrollListResponse {
  ...
  worker_count: number;
  entry_count: number;
  gph_payments_count: number;
  gph_payments_gross: number;
  gph_payments_net: number;
...
export interface GPHPaymentCalculationSnapshot {
  input: { gross_amount: string; is_astana_hub: boolean; apply_mrp_deduction?: boolean };
...
export type GPHPaymentStatus = "pending" | "approved" | "paid";

export interface ConstantsUsed {
  year: number;
  currency: string;
  mzp: number;
  mrp: number;
...
export interface GPHPayment {
  ...
  so: string;
  total_cost: string;
  status: GPHPaymentStatus;
  apply_mrp_deduction: boolean;
  is_astana_hub: boolean;
  can_approve: boolean;
  can_mark_paid: boolean;
  approved_at: string | null;
  approved_by: GPHPaymentManager | null;
  paid_at: string | null;
...
export interface ApproveGPHDto {
  apply_mrp_deduction?: boolean;
}
